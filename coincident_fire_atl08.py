...
def coin(fire_df, atl08_df, atl08_year):
    ...
    up = False
    joined = gpd.sjoin(fire_df, atl08_df[['geometry']],
...
def filter_fires(f_df):
    ...
    f_df['yint'] = f_df['atl08_years'].apply(lambda years: [int(y) for y in years.split(',')])
...
def run_coin(fire_path, tile_num, atl08_paths, atl08_years, hls_path):
    local_atl08_paths, local_atl08_years, local_hls_path = download_(tile_num, atl08_paths, atl08_years, hls_path)
    ...
    for a, y in zip(local_atl08_paths, local_atl08_years):
        print(f'finding intersections of {Path(a).stem} and {Path(fire_path).stem} for year {y}')
...
if __name__ == "__main__":
    ...
    parse.add_argument("--fire_path", help="fire polygons geopackage with a YEAR attribute", required=True)
    parse.add_argument(
        "--atl08_paths",
        nargs='+',

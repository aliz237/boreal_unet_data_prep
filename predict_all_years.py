...
def predict_raster_all_years(tile_num, hls_tindex, topo_path, lc_path, model_path,
    ...
    hls_path_year = df[df['tile_num'] == tile_num][['s3_path', 'year']].to_dict(orient='records')
    print(hls_path_year)
...
if __name__ == "__main__":
    ...
    parse.add_argument("--hls_tindex", help="HLS tindex with s3_path and tile_num attributes", required=True)
    parse.add_argument(
    ...
    parse.add_argument("--output_dir", help="output predicted raster path", required=False, default='output'),
    parse.add_argument("--input_dir", help="input dir to download data", required=False, default='input'),
    parse.add_argument("--model_path", help="path to UNet model", required=True)
    parse.add_argument("--patch_size", help="patch size, should be the same as what was used when training the model",
                       type=int, default=128)
    parse.add_argument("--step_size", help="step size for sliding the window of size patch_size over the input rasters",
                       type=int, default=100)
    parse.add_argument("--ndval", help="nodata value", type=int, default=-9999)
    parse.add_argument("--nodata_thresh", help="predict a patch if nodata fraction is below nodata_thresh",
                       type=float, default=0.05)
    parse.add_argument("--max_na_block",

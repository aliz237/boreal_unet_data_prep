...
def test_per_year_writes_one_record_per_year_when_coverage_is_sufficient(tmp_path):
    hls_2019, atl08_2019, topo_arr = _write_year(tmp_path, 2019)
    hls_2020, atl08_2020, _ = _write_year(tmp_path, 2020)
    hls_2021, atl08_2021, _ = _write_year(tmp_path, 2021)
    topo_path = write_gtiff(tmp_path / 'topo.tif', topo_arr)

    out_path = Path(tmp_path / 'peryear_16_0.tfrecord.gz')
    extract_patches_tfrec_per_year(
        hls_paths={2019: str(hls_2019), 2020: str(hls_2020), 2021: str(hls_2021)},
        atl08_paths={2019: str(atl08_2019), 2020: str(atl08_2020), 2021: str(atl08_2021)},
        topo_path=str(topo_path),
        tfrecord_path=out_path,
    ...
    written = list(tmp_path.glob('peryear_16_0_*.tfrecord.gz'))
    assert len(written) == 1
    assert written[0].name.endswith('_3.tfrecord.gz')  # one record per year, not per pair
    ...
    # (patch, patch, hls_bands + topo_bands + atl08_bands) = (16, 16, 2+1+1), no time axis
    for record in records:
        assert record.shape == (PATCH_SIZE, PATCH_SIZE, 4)


def test_per_year_drops_patch_and_removes_file_when_lidar_is_too_sparse(tmp_path):
    hls_2019, atl08_2019, topo_arr = _write_year(tmp_path, 2019, atl08_valid_frac=0.0)
    topo_path = write_gtiff(tmp_path / 'topo.tif', topo_arr)

    out_path = Path(tmp_path / 'peryear_sparse_16_0.tfrecord.gz')
    extract_patches_tfrec_per_year(
        hls_paths={2019: str(hls_2019)},
        atl08_paths={2019: str(atl08_2019)},
        topo_path=str(topo_path),
        tfrecord_path=out_path,
        patch_size=PATCH_SIZE,
        overlap=0,
    ...
    assert not out_path.exists()
    assert list(tmp_path.glob('peryear_sparse_16_0*.tfrecord.gz')) == []


def test_per_year_drops_patch_when_hls_coverage_is_too_sparse(tmp_path):
    hls_2019, atl08_2019, topo_arr = _write_year(tmp_path, 2019, hls_valid=False)
    topo_path = write_gtiff(tmp_path / 'topo.tif', topo_arr)

    out_path = Path(tmp_path / 'peryear_nohls_16_0.tfrecord.gz')
    extract_patches_tfrec_per_year(
        hls_paths={2019: str(hls_2019)},
        atl08_paths={2019: str(atl08_2019)},
        topo_path=str(topo_path),
    ...
    assert not out_path.exists()
    assert list(tmp_path.glob('peryear_nohls_16_0*.tfrecord.gz')) == []


def test_per_year_fire_years_skips_years_without_fire_coverage(tmp_path):
    hls_2019, atl08_2019, topo_arr = _write_year(tmp_path, 2019)
    hls_2020, atl08_2020, _ = _write_year(tmp_path, 2020)
    hls_2021, atl08_2021, _ = _write_year(tmp_path, 2021)
    topo_path = write_gtiff(tmp_path / 'topo.tif', topo_arr)

    out_path = Path(tmp_path / 'peryear_fire_16_0.tfrecord.gz')
    extract_patches_tfrec_per_year(
        hls_paths={2019: str(hls_2019), 2020: str(hls_2020), 2021: str(hls_2021)},
        atl08_paths={2019: str(atl08_2019), 2020: str(atl08_2020), 2021: str(atl08_2021)},
        topo_path=str(topo_path),
        ...
        patch_size=PATCH_SIZE,
        overlap=0,
        fire_years={2019, 2021},  # 2020 has no fire coverage, so it's skipped
    )

    written = list(tmp_path.glob('peryear_fire_16_0_*.tfrecord.gz'))
    assert len(written) == 1
    assert written[0].name.endswith('_2.tfrecord.gz')
